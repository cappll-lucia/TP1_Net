...
namespace Data.DataBase
{
    public class AlumnoAdapter : Adapter
    {
        ...
        public void Delete(int leg)
        {
            ...
            {
                this.OpenConnection();
                string consulta = "delete from estudiantes where legajo=@leg";
                SQLiteCommand comando = new SQLiteCommand(consulta, sqliteConn);
                comando.Parameters.Add("@leg", DbType.Int32).Value = leg; //revisar que DBType sea correcto pq no encuentro SQLiteDbType
                SQLiteDataReader reader = comando.ExecuteReader();
        ...
        public void Insert(Alumno alu)
        {
            try
            {
                ...
                comandoSave.Parameters.Add("@carrera", DbType.Int32).Value = alu.IdCarrera;
        ...
        public void Update(Alumno alu)
        {
            ...
            {
                this.OpenConnection();
                string consulta = "update estudiantes set nombre = @nombre, apellido = @apellido , idCarrera = @carrera , estado = @estado where legajo = @leg"; //tira error esta sentencia en sqlite
                SQLiteCommand comando = new SQLiteCommand(consulta, sqliteConn);
                ...
                comando.Parameters.Add("@carrera", DbType.Int32).Value = alu.IdCarrera;
                comando.Parameters.Add("@estado", DbType.String).Value = alu.Estado;
                comando.Parameters.Add("@leg", DbType.Int32).Value = alu.Legajo;    
                comando.ExecuteNonQuery();
            }
            catch (SQLiteException Ex1)
            {
